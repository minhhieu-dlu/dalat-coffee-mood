...
const vibeTags = ['Chill', 'Acoustic', 'Rừng núi', 'Làm việc']
...
export default function RegisterPage() {
  const [state, formAction] = useActionState(
    async (_previousState: AuthState, formData: FormData) => signUpAction(formData),
    initialState
  )
  ...
  return (
    <main className="relative min-h-[calc(100vh-5rem)] overflow-hidden px-4 py-6 md:py-10">
      <div className="absolute inset-0">
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(10,47,29,0.58),rgba(10,47,29,0.25))]" />
        <div
      ...
      <section className="relative mx-auto flex min-h-[calc(100vh-6rem)] w-full max-w-md items-center">
        <div className="w-full rounded-4xl border border-white/20 bg-white/16 p-5 text-white shadow-[0_24px_80px_rgba(0,0,0,0.28)] backdrop-blur-2xl sm:p-6">
          ...
          <div className="rounded-[1.75rem] border border-white/15 bg-pine-dark/20 p-4 shadow-[0_20px_60px_rgba(0,0,0,0.18)] backdrop-blur-xl sm:p-5">
            <div className="mb-5">
              <h1 className="text-2xl font-bold tracking-tight text-white">Đăng ký</h1>
              <p className="mt-2 text-sm leading-6 text-white/75">
                Tạo tài khoản để lưu mood, vibe và những quán cà phê bạn yêu thích.
              </p>
            </div>
            ...
            <form action={formAction} className="space-y-4">
              ...
              <div>
                <p className="mb-2 text-sm font-medium text-white/90">Vibe yêu thích</p>
                    ...
                    return (
                      <button
                        ...
                        className={[
                          ...
                          active
                            ? 'bg-pine-dark text-white shadow-[0_12px_24px_rgba(10,47,29,0.25)]'
                            : 'border border-white/20 bg-white/90 text-pine-dark hover:bg-white',
                        ].join(' ')}
                      >
                        {vibe}
                      </button>
            ...
            <p className="mt-5 text-center text-sm text-white/80">
              Đã có tài khoản?{' '}
              <Link href="/login" className="font-semibold text-white underline decoration-white/40 underline-offset-4 transition hover:text-pine-light">
                Đăng nhập
              </Link>
            </p>
          </div>
